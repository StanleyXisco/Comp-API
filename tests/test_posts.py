...
def test_get_one_post_not_exist(authorized_client, test_posts):
    response = authorized_client.get(f"/posts/245698")
...
@pytest.mark.parametrize("title, content, published", [
    ("new title", "first content", True),
    ("favorite title", "Epl content", False),
    ("tallest title", "wahoo content", True)
])
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    response = authorized_client.post("/posts/", json={"title": "new title", "content": "first content"})
...
def test_unauthorized_user_create_post(client, test_posts, test_user):
    response = client.post("/posts/", json={"title": "new title", "content": "first content", "owner_id": test_user['id']})
    assert response.status_code == 401
...
def test_delete_post_non_existent(authorized_client, test_posts, test_user):
    response = authorized_client.delete(f"/posts/245698")
    assert response.status_code == 404

def test_delete_other_user_post(authorized_client, test_posts, test_user):
    

    # try to delete a post that belongs to the other user
...
def test_update_post_non_existent(authorized_client, test_posts, test_user):
    ...
    response = authorized_client.put(f"/posts/245698", json=data)
