...
def test_delete_non_existent_vote(authorized_client, test_posts, test_user):
    response = authorized_client.post("/vote/", json={"post_id": test_posts[0].id, "dir": 0})
    assert response.status_code == 404

def test_vote_on_non_existent_post(authorized_client, test_posts, test_user):
    response = authorized_client.post("/vote/", json={"post_id": 245698, "dir": 1})
    assert response.status_code == 404

def test_unauthorized_user_vote_on_post(client, test_posts, test_user):
